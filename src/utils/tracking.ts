...
class WidgetTracker {
    ...
    private constructor() {
        this.trackingEndpoint = 'https://chat-bot-widget-taupe.vercel.app/api/track-widget';
        this.isEnabled = true;
    }
    ...
    async trackInstallation() {
        ...
        try {
            ...
            if (!response.ok) {
                console.warn('Failed to track widget installation', response.status, await response.text());
            } else {
